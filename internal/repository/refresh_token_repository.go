...
func (repo *RefreshTokenRepository) DeleteByHash(hash string) error {
	if hash == "" {
		return nil
	}
	return repo.db.Delete(&domain.RefreshToken{}, "token_hash = ?", hash).Error
}

func (repo *RefreshTokenRepository) DeleteExpired(now time.Time) error {
	return repo.db.Where("expires_at <= ?", now).Delete(&domain.RefreshToken{}).Error
}
